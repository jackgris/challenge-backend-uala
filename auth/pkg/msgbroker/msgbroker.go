...
loop:
	for {
		select {
		case <-ctx.Done():
			m.logs.Info(ctx, "auth service", "publish message", "conxtext timeout", "message ID", msg.UUID)
			break loop
		default:
			if err := m.pub.Publish(topic, msg); err != nil {
				m.logs.Info(ctx, "auth service", "", "publish message error", err, "message ID", msg.UUID)
			} else {
				break loop
			}
		}
